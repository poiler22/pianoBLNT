        ...
        if event.type == pygame.KEYDOWN:
            if ( event.key == 27 ):
                ...
                if pygame.key.get_mods() & pygame.KMOD_SHIFT:
                    ...
                    print('self.selectcursorselection() is' + str(self.selectcursorselection()))
                    if self.selectednotes:
                        for note in self.selectednotes:
                            # hit the key:
                            print('what keys?: ' + str(note) )
                            self.keymusic.hitkey( midi, note[0],  note[1], self.tickstosecs(note[2]),
